...
const Cart = () => {
    ...
    const handleWhatsAppCheckout = () => {
        ...
        let message = `Bless ups! I'd like to place an order from IslandLeaf:%0A%0A`;
        cart.forEach(item => {
            message += `- ${item.name} (Qty: ${item.quantity}) = JMD ${(item.price * item.quantity).toLocaleString()}%0A`;
        });
        message += `%0A*Total: JMD ${cartTotal.toLocaleString()}*`;
    ...
    if (cart.length === 0) {
        return (
            <div className={`cart-empty container ${isLoaded ? 'fade-in' : ''}`}>
                ...
                <h2>Your Selection Awaits</h2>
                <p>Your cart is empty. Browse our dispensary for premium Jamaican strains.</p>
                <Link to="/products" className="btn btn-primary glow">Browse Dispensary</Link>
            </div>
        );
    }
    ...
    return (
        <div className={`cart-page container ${isLoaded ? 'fade-in' : ''}`}>
            <div className="editorial-header">
                ...
                <p>Review your curated strains before checkout.</p>
            </div>
                    ...
                    {cart.map(item => (
                        <div key={item.id} className="premium-cart-item glass">
                            ...
                            <div className="item-price-side">
                                ...
                                <span className="price-val">JMD {(item.price * item.quantity).toLocaleString()}</span>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="cart-checkout-panel">
                    <div className="checkout-card glass sticky-card">
                        ...
                        <div className="summary-list">
                            ...
                            <div className="summary-line">
                                <span>Delivery</span>
                                <span className="free-text">Free Island-Wide</span>
                            </div>
                        ...
                        <div className="checkout-trust">
                            <div className="trust-bit">
                                <ShieldCheck size={14} /> <span>Discreet Delivery</span>
                            </div>
                            <div className="trust-bit">
                                <Truck size={14} /> <span>Island-Wide Shipping</span>
                            </div>
                            <div className="trust-bit">
                                <RefreshCcw size={14} /> <span>Quality Guarantee</span>
                            </div>
                        </div>
                    </div>
                </div>
